					...
					int &depth, int &cases)
{
			...
			6)
	{
		...
		copy &= ~(7 << (y * 9 + (x + 1) * 3));
		copy &= ~(7 << (y * 9 + (x - 1) * 3));
		res = (res + calculate(copy, moves, depth)) % (1 << 30);
	}
...
void past_2_maps(int **map1, int **map2)
{
	map1[0][0] += map2[0][0];
	map1[0][1] += map2[0][1];
	map1[0][2] += map2[0][2];
...
s_array calculate(uint64_t &board, int moves, int &depth)
{
	...
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			...
			{
				cases = 0;
				res = (res + calculate_cases(copy, board, i, j, moves, depth, cases)) % (1 << 30);
				if (cases == 0)
				{
					copy = board;
					copy |= (1 << (i * 9 + j * 3));
					res =
						(res + calculate(copy, moves, depth)) % (1 << 30);
				}
			}
		}
	}
